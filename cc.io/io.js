...
/////////////////////////////////////////////////////////////////////////////////////
(function() {
    var require = define.getRequire("cc.io.1.0.2/");
    define.parameters = {};
    define.parameters.system = "pkx";
    define.parameters.id = "cc.io.1.0.2/";
        ...
        function IO(pkx, module) {
            ...
            this.Stream = {
                ...
                readAsString : function () {
                    var own = this;
                    return new Promise(function (resolve, refuse) {
                        own.seek(0).then(function () {
                            own.read().then(function (bytes) {
                                try {
                ...
                getMimeType : function () {
                    var own = this;
                    return new Promise(function (resolve, refuse) {
                        var origPos = own.getPosition();
                        own.seek(0).then(function () {
                            own.read(4).then(function (arr) {
                                own.seek(origPos).then(function () {
                                    try {
                                        var header = "";
            ...
            this.BufferedStream = function (buffer, name) {
                ...
                var closed = false;
                var position = 0;
                var seek_origin = self.SEEKORIGIN_BEGIN;
                ...
                this.getPosition = function () {
                    return new Promise(function (resolve, refuse) {
                        closed ? refuse(new Error(self.ERROR_STREAM_CLOSED, "")) : resolve(position);
                    });
                };
                ...
                this.seek = function (pos, origin) {
                    ...
                    return new Promise(function (resolve, refuse) {
                        ...
                        switch (seek_origin) {
                            case self.SEEKORIGIN_BEGIN:
                                position = pos;
                                break;
                            case self.SEEKORIGIN_CURRENT:
                                position += pos;
                                break;
                            case self.SEEKORIGIN_END:
                    ...
                    return new Promise(function (resolve, refuse) {
                        ...
                        switch (seek_origin) {
                            ...
                            case self.SEEKORIGIN_END:
                                position = own.getLength().then(function (len) {
                                    position = len - pos;
                                    stream.seek(offset + position, seek_origin).then(resolve, refuse);
                                }, refuse);
                                return;
                            default:
                                refuse(new Error("Unsupported seek origin '" + seek_origin + "'."));
                ...
                this.read = function (len) {
                    return new Promise(function (resolve, refuse) {
                        ...
                        function doRead() {
                            ...
                            stream.read(len).then(resolve, refuse);
                        }
                    });
                };
                this.write = function (data) {
                    return new Promise(function (resolve, refuse) {
                        ...
                        if (!init) {
                            own.seek(0).then(doWrite, refuse);
                        }
                        else {
                            doWrite();
                        }
                        ...
                        function doWrite() {
                            ...
                            stream.write(data).then(resolve, refuse);
                        }
                    });
                };
                this.close = function () {
            ...
            this.protocols.register = function (config) {
                ...
                if (protocols[config.protocol]) {
                    throw "Protocol '" + protocol + "://' is already registered to module '" + protocols[protocol].package.getFullName() + "." + protocols[protocol].module.getName() + "'.";
                }
                ...
                if (typeof config.formats !== "undefined") {
                    ...
                    for (var i = 0; i < config.formats.length; i++) {
                        ...
                        if (formats[config.formats[i]]) {
                            throw "Format '" + config.formats[i] + "' is already registered to module '" + protocols[protocol].package.getFullName() + "." + protocols[protocol].module.getName() + "'.";
                        }
                    }
                }
...
//# sourceURL=http://cc.io.1.0.2/io.js
